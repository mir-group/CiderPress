...
class OmegaMap(FeatureNormalizer):
    def __init__(self, i_n, i_s, i_alpha, c, B, C, bounds=None):
        self.i_n = i_n  # index for electron density
        self.i_s = i_s  # index for s^2
        self.i_alpha = i_alpha  # index for alpha
        self.c = c
        self.B = B
        self.C = C
        self._bounds = bounds or (0, 1)

    def set_current_molecule_id(self, mol_id):# 0701change
        self.current_molecule_id = mol_id# 0701change
    ...
    def fill_deriv_(self, dfdx, dfdy, x):
        ...
        inner_term = np.maximum(self.B + self.C * (alpha + 5/3 * s2), 1e-10)
        omega = np.sqrt(n**(2/3) * inner_term)
        denom = np.maximum(1 + self.c * omega, 1e-10)
